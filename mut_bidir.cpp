...
private:
    ...
    std::optional<Dist> create_delete_range_dist(const Path& x, int kd) const {
        ...
        for (int i = 0; i <= xn - kd; i++) {
            const int j = i + kd - 1;
            // If the next/previous vertex is specular, the same vertex is generated. So skip it.
            if (i > 0 && x.vs[i-1].is_specular(scene_)) {
                dist.add(0_f);
            }
            else if (j < xn - 1 && x.vs[j+1].is_specular(scene_)) {
                dist.add(0_f);
            }
            // If the range include the unconnectable primary edge, skip it.
            else if (i == 1 && !x.vs[0].is_connectable_endpoint(scene_)) {
                dist.add(0_f);
            }
            else if (i == xn - 2 && !x.vs[xn - 1].is_connectable_endpoint(scene_)) {
                dist.add(0_f);
            }
            else {
                dist.add(1_f);
